...
_PyLong_Negate(PyBigIntObject **x)
{
    assert(Py_REFCNT(*x) == 1);
    (*x)->tagged_size = -(*x)->tagged_size;
    return;
}
...
static PyLongObject *
long_normalize(PyBigIntObject *v)
{
    ...
    if (i < 3) {
        Py_ssize_t val = v->ob_digit[0] + (v->ob_digit[1]<<PYLONG_BITS_IN_DIGIT);
        if (Py_SIZE(v) < 0) {
            val = -val;
        }
        PyLongObject *small = get_small_int(val);
        Py_DECREF(v);
        return small;
    }
    return v;
...
_PyLong_New(Py_ssize_t size)
{
    ...
    result = PyObject_MALLOC(offsetof(PyBigIntObject, ob_digit) +
                             size*sizeof(digit));
    if (!result) {
        PyErr_NoMemory();
        return NULL;
