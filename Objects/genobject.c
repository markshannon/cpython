...
_PyGen_Finalize(PyObject *self)
{
    ...
    if (gen->gi_code != NULL &&
        ...
        _PyFrame_GetLasti(gen->gi_frame) < 0)
    {
...
_PyGen_yf(PyGenObject *gen)
{
    ...
    if (f && f->f_stacktop) {
        ...
        if (code[(_PyFrame_GetLasti(f) + 1)*sizeof(_Py_CODEUNIT)] != YIELD_FROM)
            return NULL;
        yf = f->f_stacktop[-1];
        Py_INCREF(yf);
    }
           ...
           PyObject *typ, PyObject *val, PyObject *tb)
{
    ...
    if (yf) {
        ...
        if (!ret) {
            ...
            assert(_PyFrame_GetLasti(gen->gi_frame) >= 0);
            gen->gi_frame->f_lastinst += 1;
