                 ...
                 PyInterpreterState *interp, uint64_t id, int whence)
{
    ...
    tstate->stack.datastack_chunk = NULL;
    tstate->stack.datastack_top = NULL;
    tstate->stack.datastack_limit = NULL;
    tstate->what_event = -1;
...
{
    _PyStackChunk *chunk = tstate->stack.datastack_chunk;
    tstate->stack.datastack_chunk = NULL;
    while (chunk != NULL) {
...
push_chunk(PyThreadState *tstate, int size)
{
    ...
    _PyStackChunk *new = allocate_chunk(allocate_size, tstate->stack.datastack_chunk);
    if (new == NULL) {
        return NULL;
    }
    if (tstate->stack.datastack_chunk) {
        tstate->stack.datastack_chunk->top = tstate->stack.datastack_top -
                                       &tstate->stack.datastack_chunk->data[0];
    }
    tstate->stack.datastack_chunk = new;
    tstate->stack.datastack_limit = (PyObject **)(((char *)new) + allocate_size);
    // When new is the "root" chunk (i.e. new->previous == NULL), we can keep
    // _PyThreadState_PopFrame from freeing it later by "skipping" over the
    // first element:
    PyObject **res = &new->data[new->previous == NULL];
    tstate->stack.datastack_top = res + size;
    return res;
}
...
_PyThreadState_PushFrame(PyThreadState *tstate, size_t size)
{
    ...
    if (_PyThreadState_HasStackSpace(tstate, (int)size)) {
        _PyInterpreterFrame *res = (_PyInterpreterFrame *)tstate->stack.datastack_top;
        tstate->stack.datastack_top += size;
        return res;
    }
...
{
    assert(tstate->stack.datastack_chunk);
    PyObject **base = (PyObject **)frame;
    if (base == &tstate->stack.datastack_chunk->data[0]) {
        _PyStackChunk *chunk = tstate->stack.datastack_chunk;
        _PyStackChunk *previous = chunk->previous;
        // push_chunk ensures that the root chunk is never popped:
        assert(previous);
        tstate->stack.datastack_top = &previous->data[previous->top];
        tstate->stack.datastack_chunk = previous;
        _PyObject_VirtualFree(chunk, chunk->size);
        tstate->stack.datastack_limit = (PyObject **)(((char *)previous) + previous->size);
    }
    else {
        assert(tstate->stack.datastack_top);
        assert(tstate->stack.datastack_top >= base);
        tstate->stack.datastack_top = base;
    }
}
