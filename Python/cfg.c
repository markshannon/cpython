...
int _PyCfg_CopyBlock(ControlFlowGraph *cfg,  int block) {
    ...
    int instr_count = cfg->blocks[block].b_end - cfg->blocks[block].b_start;
    if (cfg->instruction_count + instr_count > cfg->allocated_instructions) {
        if (cfg->allocated_instructions*2 > MAX_INSTRUCTIONS) {
            PyErr_NoMemory();
            return -1;
        }
        Instruction *tmp = 
            PyObject_Realloc(cfg->instructions, sizeof(Instruction) * cfg->allocated_instructions*2);
        if (tmp == NULL) {
            PyErr_NoMemory();
            return -1;
        }
        cfg->instructions = tmp;
        cfg->allocated_instructions *= 2;
    }
    cfg->blocks[copy] = cfg->blocks[block];
    cfg->blocks[copy].b_start = cfg->instruction_count;
    for (int i = 0; i < instr_count; i++) {
        cfg->instructions[cfg->instruction_count+i] = cfg->instructions[cfg->blocks[block].b_start+i];
    }
    cfg->instruction_count += instr_count;
    cfg->blocks[copy].b_end = cfg->instruction_count;
    return copy;
}
...
ControlFlowGraph *createCfg(Instruction *instructions, int count, int allocated) {
    ...
    cfg->allocated_instructions = allocated;
...
void cfgDump(ControlFlowGraph *cfg) {
    ...
    for (int b = 0; b < cfg->block_count; b++) {
        ...
        printf("  Block %d (offset=%d, reachable=%d, exit=%d, size=%d):\n", b, bb->assembler_offset, bb->is_reachable, bb->is_exit, bb->assembler_size);
        for (int i = bb->b_start; i < bb->b_end; i++) {
            Instruction *inst = &cfg->instructions[i];
            if (inst->i_flags & INSTRUCTION_IS_BRANCH) {
                printf("    %d: %d -> %d @ %d (size = %d, flags=%d)\n", i, inst->i_opcode, inst->i_oparg, inst->i_lineno, inst->instruction_size, inst->i_flags);
            } else {
                printf("    %d: %d[%d] @ %d (size = %d, flags=%d)\n", i, inst->i_opcode, inst->i_oparg, inst->i_lineno, inst->instruction_size, inst->i_flags);
            }
        }
...
void cfgSanity(ControlFlowGraph *cfg) {
    ...
    for (int b = 0; b < cfg->block_count; b++) {
        ...
        if (bb->b_fallthrough > 0) {
            ...
            CFG_ASSERT(cfg->blocks[bb->b_fallthrough].is_reachable);
        } else {
